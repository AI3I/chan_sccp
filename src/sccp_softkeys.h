...
SCCP_API struct softKeySetConfigList softKeySetConfig;
...
struct softKeySetConfiguration {
	char name[SCCP_MAX_SOFTKEYSET_NAME];
	softkey_modes modes[SCCP_MAX_SOFTKEY_MODES];
	sccp_softkeyMap_cb_t * softkeyCbMap;                  /*!< Softkey Callback Map, ie handlers */
	SCCP_LIST_ENTRY(sccp_softKeySetConfiguration_t) list;
	boolean_t pendingDelete;
	boolean_t pendingUpdate;
	uint8_t numberOfSoftKeySets;
};
